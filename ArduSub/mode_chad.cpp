...
bool ModeChad::pos_servo_authorized(){
    ...
    return (abs(roll_diff) < g2.chad_params.roll_ctrl_threshold 
           && 
           abs(pitch_diff) < g2.chad_params.pitch_ctrl_threshold
           &&
           abs(yaw_diff) < g2.chad_params.yaw_ctrl_threshold);
}
...
void ModeChad::PID_servo(Vector3<float> measure, int dt, Vector3<float>& F){
    ...
    PIDx.set_kP(g2.chad_params.Px);
    PIDx.set_kI(g2.chad_params.Ix);
    PIDx.set_kD(g2.chad_params.Dx);

    PIDy.set_kP(g2.chad_params.Py);
    PIDy.set_kI(g2.chad_params.Iy);
    PIDy.set_kD(g2.chad_params.Dy);

    PIDz.set_kP(g2.chad_params.Pz);
    PIDz.set_kI(g2.chad_params.Iz);
    PIDz.set_kD(g2.chad_params.Dz);
